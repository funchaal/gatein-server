...
class CheckinNamespace(socketio.AsyncNamespace):
    ...
    async def on_connect(self, sid, environ, auth):
        ...
        """
        logger.debug(f"[WS] Nova tentativa de conexão. SID: {sid}")
        ...
        if not api_key:
            logger.warning("[WS] RECUSADO: Nenhuma API Key fornecida.")
        ...
        try:
            try:
                ...
                logger.warning(f"[WS] RECUSADO pela segurança: {error_detail}")
            ...
            if company.type != 'terminal':
                logger.warning(f"[WS] RECUSADO: Empresa {company.id} não é um terminal.")
            ...
            logger.info(f"[WS] SUCESSO! Terminal conectado. Terminal ID: {terminal_id_str}")
    ...
    async def on_disconnect(self, sid):
        ...
        """
        logger.info(f"[WS] Conexão encerrada/perdida. SID: {sid}")
        ...
        for terminal_id, saved_sid in list(active_terminals.items()):
            if saved_sid == sid:
                ...
                logger.info(f"[WS] Limpeza feita! Terminal {terminal_id} removido dos ativos.")
                break
